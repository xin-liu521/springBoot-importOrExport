drop procedure if exists salary_order_new;
DELIMITER //
	CREATE PROCEDURE salary_order_new(
...
BEGIN
   ...
   FROM salary_user_info s_u_i WHERE s_u_i.record_id=param_month_record AND s_u_i.is_new_rider=0;
    -- 定义游标获取不到异常
           ...
           BEGIN
                  ...
                  DECLARE variable_complete_proportion,
                           variable_on_time_proportion  varchar(11);
								 ...
								 SET variable_insurance_price=0, variable_interval_price=0;
                     ...
                     SELECT s.rider_name,s.stataion_id,s.station_name
										 ...
										 -- 大夜单量
										 SET variable_big_night_number=(SELECT count(*) FROM salary_order_detail s WHERE  s.record_id=param_month_record AND s.rider_id=rider_id AND
																					DATE_FORMAT(order_time,'%T')>='23:00:00' AND s.delivery_price>0);
										 SET variable_big_night_number=variable_big_night_number+(SELECT count(*) FROM salary_order_detail s WHERE  s.record_id=param_month_record AND s.rider_id=rider_id AND
										 ...
										 INSERT INTO salary_bj_result_detail (record_id,
																											 ...
																											 interval_two_number,-- 大于等于90小于200单量
                             ...
                             variable_station_name,
														...
														(SELECT count(*) FROM salary_order_detail s WHERE  s.record_id=param_month_record AND s.rider_id=rider_id AND
																					DATE_FORMAT(order_time,'%T')>='21:00:00' AND DATE_FORMAT(order_time,'%T')<'23:00:00' AND s.delivery_price>0),
														 -- 大夜单量
														 variable_big_night_number,
														(SELECT count(*) FROM salary_order_detail s WHERE s.record_id=param_month_record AND s.rider_id=rider_id AND s.order_price >=90 AND s.order_price <200 AND s.delivery_price>0 ),
                              ...
                              ifnull(s_o_a.serious_overtime_number,0),
											...
											-- 往月入职骑手  单量,出勤考核不达标 不发(底薪和补助)
											IF variable_number<=400 AND variable_attend_actual>25 AND (YEAR(entry_time)!=@year OR month(entry_time)!=@month)
											THEN
												 -- 小于等于400单薪资
												 SET variable_less_task_price=variable_number*700;
											ELSE   -- 单量,出勤考核达标或者是当月入职新骑手
												 ...
												 END IF;
												 -- 往月入职骑手考核各种率
												 IF  (YEAR(entry_time)!=@year OR month(entry_time)!=@month)
												 AND (
												  ABS(left(variable_complete_proportion,length(variable_complete_proportion)-1))<99
												  OR ABS(left(variable_on_time_proportion,length(variable_on_time_proportion)-1))<96
												  )
												 THEN
														 SET variable_no_standard_price=-(variable_number*30);
												 END IF;
											END IF;
											...
											BEGIN
												...
												DECLARE count_intervals cursor for SELECT s_p.interval_min,s_p.interval_max,value FROM salary_param s_p
															 WHERE s_p.param_code='count_interval' AND s_p.ruleid=2 ORDER BY s_p.interval_min DESC;
												-- 定义游标获取不到异常
											...
											UPDATE salary_bj_result_detail s_r_d  SET
												 ...
												 s_r_d.night_price=s_r_d.night_number*100,
												 s_r_d.big_night_price=s_r_d.big_night_number*200,
												 s_r_d.interval_price=variable_interval_price,
												 s_r_d.interval_two_price=s_r_d.interval_two_number*200,
												 s_r_d.interval_three_price=s_r_d.interval_three_number*400,
												 s_r_d.interval_four_price=s_r_d.interval_four_number*800,
												 s_r_d.interval_five_price=s_r_d.interval_five_number*1200,
												 s_r_d.diatance_one_price=s_r_d.distance_one_number*100,
												 s_r_d.distance_two_price=s_r_d.distance_two_number*150,
